...
public class CallService extends Service {
    ...
    private final LinkedBlockingQueue<byte[]> musicQ = new LinkedBlockingQueue<>(150);
    ...
    public void startSession(InetAddress peer, boolean host) {
        ...
        try { setupAudio(); } catch (Exception e) { e.printStackTrace(); return; }
    ...
    private void setupAudio() throws Exception {
        ...
        if (recBufSize == AudioRecord.ERROR_BAD_VALUE || recBufSize == AudioRecord.ERROR)
            throw new Exception("Bad AudioRecord config");
        recBufSize = Math.max(recBufSize, 4096);

        recorder = new AudioRecord(
            MediaRecorder.AudioSource.VOICE_COMMUNICATION,
            SAMPLE_RATE, CHANNEL_IN, FORMAT, recBufSize);
        if (recorder.getState() != AudioRecord.STATE_INITIALIZED)
            throw new Exception("AudioRecord not initialized");

        int playBufSize = AudioTrack.getMinBufferSize(SAMPLE_RATE, CHANNEL_OUT, FORMAT);
        playBufSize = Math.max(playBufSize, 4096);

        voicePlayer = new AudioTrack(
            AudioManager.STREAM_VOICE_CALL,
            SAMPLE_RATE, CHANNEL_OUT, FORMAT,
            playBufSize, AudioTrack.MODE_STREAM);
        if (voicePlayer.getState() != AudioTrack.STATE_INITIALIZED)
            throw new Exception("AudioTrack (voice) not initialized");

        int musicBufSize = AudioTrack.getMinBufferSize(SAMPLE_RATE, CHANNEL_OUT, FORMAT);
        musicBufSize = Math.max(musicBufSize, 4096);

        musicPlayer = new AudioTrack(
            AudioManager.STREAM_MUSIC,
            SAMPLE_RATE, CHANNEL_OUT, FORMAT,
            musicBufSize, AudioTrack.MODE_STREAM);
    ...
    private void voiceSendLoop() {
        try {
            ...
            while (running.get()) {
                if (recorder == null || muted || peerAddr == null) {
                ...
                if (n > 0) {
                    ...
                    voiceSendSock.send(new DatagramPacket(pkt, n, peerAddr, Protocol.PORT_VOICE));
                }
            }
